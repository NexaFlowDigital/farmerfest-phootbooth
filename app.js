...
(() => {
  ...
  const emailBtnLabel     = $("emailBtnLabel");
  ...
  async function captureWithOverlay(){
    ...
    // NOTE: We intentionally do NOT mirror the saved image.
    // The live preview is mirrored via CSS (feels natural like a mirror),
    // but the OUTPUT stays un-mirrored so that any text on the subject's
    // clothing reads correctly in the final photo.
  ...
  async function drawSquareFrameToCanvas(ctx, size){
    ...
    // Un-mirrored output — see note in captureWithOverlay().
  ...
  function resetEmailPanel(){
    ...
    emailBtnLabel.textContent = "SEND TO MY EMAIL";
    setEmailStatus(null);
  }
  ...
  /**
   * Parse the email input which may contain multiple addresses separated
   * by commas, semicolons, or whitespace. Returns { valid: [...], invalid: [...] }.
   */
  function parseEmails(raw){
    ...
    for (const t of tokens){
      if (isEmailValid(t)) valid.push(t);
      else                 invalid.push(t);
    }
  ...
  function setEmailStatus(kind, html){
    if (!emailStatus) return;
    emailStatus.classList.remove("show", "sending", "success", "error");
    if (!kind){ emailStatus.innerHTML = ""; return; }
    emailStatus.classList.add("show", kind);
  ...
  async function emailResult(){
    ...
    if (!GAS_POST_URL || GAS_POST_URL.startsWith("REPLACE_")){
      setEmailStatus("error", "Email isn't configured yet. Set GAS_POST_URL in config.js.");
      return;
    }

    // Parse + validate addresses
    const { valid, invalid } = parseEmails(emailInput.value);
    if (!valid.length){
      setEmailStatus("error", "Please enter at least one valid email address.");
      emailInput.focus();
      return;
    }
    if (invalid.length){
      setEmailStatus("error",
        `These look invalid: ${invalid.map(escapeForHtml).join(", ")}. Please check and try again.`);
      emailInput.focus();
      return;
    }

    // INSTANT feedback (the key UX fix — show this BEFORE any await)
    emailBtn.disabled    = true;
    emailInput.disabled  = true;
    emailBtnLabel.textContent = "SENDING…";
    setEmailStatus("sending",
      `<span class="spinner" aria-hidden="true"></span>Sending to ${valid.length} ${valid.length === 1 ? "address" : "addresses"}…`);
    ...
    try{
      ...
      if (lastResultType === "photo"){
        ...
        payloadObj = {
          emails: valid,
          email: valid[0], // kept for backwards compat with old GAS deployments
          type: "photo",
          filename: `FarmerFest_PhotoStrip_${niceStamp()}.png`,
          mimeType: "image/png",
      ...
      // text/plain avoids CORS preflight that Apps Script Web Apps don't answer
      ...
      // SUCCESS — clear the field, show a clear success banner
      setChip("ok", "Email sent");
      emailInput.value = "";
      const list = valid.map(escapeForHtml).join(", ");
      setEmailStatus("success",
        `✓ Sent! Check your inbox (and spam, just in case).<br><span style="font-weight:500;opacity:.85;">${list}</span>`);
      emailBtnLabel.textContent = "✓ SENT!";

      // After a moment, restore the button + clear status so a new email can be entered
      setTimeout(() => {
        emailBtnLabel.textContent = "SEND TO ANOTHER EMAIL";
        emailBtn.disabled = false;
        emailInput.disabled = false;
      }, 1800);
      setTimeout(() => {
        // only clear the banner if the user hasn't started typing again
        if (!emailInput.value.trim()) setEmailStatus(null);
      }, 8000);
      ...
      setEmailStatus("error",
        "Sorry — sending failed. Please try again, or download and share manually.");
      emailBtnLabel.textContent = "SEND TO MY EMAIL";
      emailBtn.disabled = false;
      emailInput.disabled = false;
    }
  ...
  // When a chip is tapped, append (or replace) the @domain part of the
  // CURRENT address being typed. Handles two cases:
  //   "joe"          + @gmail.com  →  "joe@gmail.com"
  //   "joe@gma"      + @gmail.com  →  "joe@gmail.com"   (replaces partial)
  //   "joe@x.com, "  + @gmail.com  →  "joe@x.com, @gmail.com" (new address)
  // After applying, advance the cursor to end and let the user keep typing more.
  if (domainChips){
    domainChips.addEventListener("click", (e) => {
      ...
      // Split on commas/semicolons keeping separators so we only modify the LAST chunk
      const parts = current.split(/([,;])/);
      let last = parts.pop() ?? "";
      const lastTrimmed = last.trimEnd();
      const trailing    = last.slice(lastTrimmed.length);

      // If the last token already has an @, replace from there. Otherwise append.
      const atIdx = lastTrimmed.indexOf("@");
      const head  = atIdx >= 0 ? lastTrimmed.slice(0, atIdx) : lastTrimmed;
      const newLast = (head + domain) + trailing;

      emailInput.value = [...parts, newLast].join("");
      emailInput.focus();
      // place cursor at end
      const v = emailInput.value;
      emailInput.setSelectionRange(v.length, v.length);

      // user is editing — clear any old status
      setEmailStatus(null);
    });
  }

  // Clear status banner once the user starts typing again
  emailInput.addEventListener("input", () => {
    if (emailStatus.classList.contains("success") || emailStatus.classList.contains("error")){
      setEmailStatus(null);
    }
  });
